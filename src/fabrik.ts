...
// Ported from the crawler sketch (sketches/crawler/src/fabrik.ts).
...
// currentDir/referenceDir are unit vectors, so their dot product is cos(angle).
// We only need to know if angle > rotor, which is dot < cos(rotor) — this avoids
// vec3.angle's acos + length sqrts on the always-run check. When the limit is
// exceeded, clamp currentDir to exactly `rotor` from referenceDir via Rodrigues
// (the rotation axis is perpendicular to referenceDir, so the formula reduces to
// ref*cos + (axis×ref)*sin), avoiding the quat→mat3→transform path.
